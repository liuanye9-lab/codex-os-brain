...
In Codex OS Brain, this is implemented as a global gated preflight: every Codex prompt is classified first, but sub-agents are used only when the task is complex, verifiable, and low-risk.
...
codex-os-brain dispatch --task "implement feature, update docs, run tests"
codex-os-brain dispatch --task "publish package safely" --json --write
