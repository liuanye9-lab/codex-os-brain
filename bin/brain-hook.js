...
async function main() {
  ...
  const handlers = {
    SessionStart: bind(handleSession), PostCompact: bind(handleSession),
    PreToolUse: bind(handleRisk), PostToolUse: bind(handleObservation), Stop: bind(handleStop),
    UserPromptSubmit: async () => ({}), PreCompact: async () => ({}),
  };
  const enabled = process.env.BRAIN_V9_HOOKS === '1' || config.hooks.enabled === true;
  const output = await dispatchHook(input, { enabled, handlers, failClosedEvents: new Set(['PreToolUse', 'Stop']), auditInternalError() {} });
