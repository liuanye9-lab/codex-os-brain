...
上面的“副驾驶、资料柜、红绿灯”是为了好懂；下面是对应的常用术语。它们不是贴标签：每一项都要能在本项目的命令、规则或验收记录里找到对应物。

| 常用术语 | 大白话 | V9 里的落点与边界 |
|---|---|---|
| **RAG（检索增强生成）** | 回答前先从资料柜挑几份可能相关的材料，而不是把整间仓库都塞进脑子。 | 可选的本地嵌入召回先选候选资料，再由 Agent 阅读和验证；Ollama 或索引不可用时退回关键词检索。召回结果是证据候选，不是指令，也不自动等于事实。 |
| **Prompt Engineering（提示词工程）** | 像办事前先写清单，把目标、红线和验收方式说清。 | 任务清单（task contract）和压缩后的随身小抄是结构化提示；只在明确约束、压缩恢复等触发条件下补充，不把长 prompt 常驻塞进每轮对话。 |
| **Context Engineering（上下文工程）** | 决定什么该带上车、什么不该占座位。 | 用有上限的 checkpoint、允许清单和检索条数控制上下文；私有原始资料不因召回而自动外发。 |
| **Loop Engineering（循环工程）** | 给 Agent 的“观察 → 动作 → 检查 → 纠偏 → 再行动”闭环装上交通规则。 | hooks 记录允许事件，失败红绿灯处理连续错误，`Stop` 用证据交卷；这不是让系统自动改写自己，而是让下一步有可核验的反馈。 |
| **数据分析** | 把零散的行车记录汇总成“哪里经常堵、哪里容易出错”的信号。 | 已脱敏的事件、失败和验收记录可以用来统计触发频率、重复失败和验证缺口，帮助调阈值与检查项；**不是一个通用 BI 平台**，也不把原始对话当作分析数据。 |
| **知识工程** | 给资料贴来源、版本和适用范围，避免一张旧便条被当成永远正确的规定。 | 任务、证据、嵌入配置和迁移都带结构、来源或指纹；旧版本资料先备份再复制核验，检索内容保持“待验证”身份。 |
| **Evaluation Engineering（评估工程）** | 不靠“我觉得做完了”，而是按题号交证据。 | 每个必做 criterion 都要对应证据；缺证据、证据失败或未知标准时，完成声明会被拒绝。 |

一句话概括：V9 把 RAG、提示词、上下文、循环、数据和知识这些能力，收进一个**默认安静、按证据介入、能回退**的本地可靠性框架，而不是让它们变成永远在线的复杂负担。
