...
test('does not treat quoted or fenced correction examples as new user corrections', () => {
  const quoted = [
    '> 你错了，不对',
    '',
    '```text',
    'You said it was fixed, but it still fails.',
    '```',
    '',
    '请分析上面的例句。',
  ].join('\n');
  assert.equal(detectCorrection(quoted).matched, false);
});

test('strips ambient host context blocks before correction detection', () => {
  const input = '<in-app-browser-context>你错了，不对</in-app-browser-context>\n'
    + '<environment_context>You said it was fixed, but it still fails.</environment_context>\n好的';
  assert.equal(stripInjectedContent(input), '好的');
  assert.equal(detectCorrection(input).matched, false);
});

test('normalizer accepts only user-visible text blocks and bounds malformed arrays', () => {
  const event = normalizeHostEvent({
    content: [
      { type: 'tool_result', text: '你错了，不对' },
      { type: 'input_text', text: '只保留这段用户文本' },
      { nested: [{ text: '不应递归读取未知结构' }] },
      null,
      42,
    ],
  }, { host: 'codex' });
  assert.equal(event.text, '只保留这段用户文本');
  assert.equal(detectCorrection(event.text).matched, false);

  const oversized = normalizeHostEvent({ content: ['a'.repeat(100_000)] }, { host: 'codex' });
  assert.ok(oversized.text.length <= 32_768);
});
