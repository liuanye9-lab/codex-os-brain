...
const { buildBehavioralContextPacket, markBehavioralContextUse } = require('../scripts/brain-lite-behavioral-context');
const { buildBehavioralTraceEvent } = require('../scripts/brain-lite-behavioral-trace');
...
test('turns passing paired samples into sanitized replay evidence', () => {
  const source = candidate({ replays: [] });
  const samples = [sample('private-case-a'), sample('private-case-b'), sample('private-case-c')];
  const result = evaluateBehavioralCandidate(source, samples, policy);
  assert.equal(result.state, 'canary');
  assert.equal(result.replays.length, 3);
  assert.ok(result.replays.every((replay) => replay.passed === true));
  assert.ok(result.replays.every((replay) => /^sample_[a-f0-9]{16}$/.test(replay.sampleRef)));
  assert.ok(!JSON.stringify(result.replays).includes('private-case'));
});
...
test('keeps a quality-safe but no-benefit experiment in replay', () => {
  const noBenefit = (id) => sample(id, {
    candidate: { passed: true, tokens: 950, durationMs: 950 },
  });
  const result = evaluateBehavioralCandidate(candidate(), [noBenefit('1'), noBenefit('2'), noBenefit('3')], policy);
  assert.equal(result.experiment.state, 'trial');
  assert.equal(result.state, 'replay');
  assert.equal(result.evaluation.lifecycleReason, 'policy-benefit-not-proven');
});

test('builds deterministic allowlisted behavioral trace metadata with a pseudonymous reference', () => {
  const source = candidate({
    rule: 'DO_NOT_PERSIST_THIS_RULE',
    scopeKey: 'internal.scope',
  });
  const options = { key: 'K7!mQ2#vR9@xL4$zN8&pT6*', now: '2026-07-13T05:00:00.000Z' };
  const first = buildBehavioralTraceEvent(source, 'evaluated', options);
  const second = buildBehavioralTraceEvent(source, 'evaluated', options);
  assert.deepEqual(first, second);
  assert.equal(first.kind, 'behavioral_memory');
  assert.match(first.candidateRef, /^bmc_[a-f0-9]{20}$/);
  assert.equal(first.behavioralAction, 'evaluated');
  assert.equal(first.behavioralState, 'candidate');
  const serialized = JSON.stringify(first);
  assert.ok(!serialized.includes(source.candidateId));
  assert.ok(!serialized.includes(source.rule));
  assert.ok(!serialized.includes(source.scopeKey));
});

test('deduplicates and redacts behavioral context while marking actual use', () => {
  const candidates = [
    candidate({ candidateId: 'bm_1', state: 'promoted', occurrences: 5, rule: 'Use access_token=DO_NOT_INJECT before delivery.' }),
    candidate({ candidateId: 'bm_2', state: 'promoted', occurrences: 4, rule: 'Use access_token=DO_NOT_INJECT before delivery.' }),
    candidate({ candidateId: 'bm_3', state: 'promoted', occurrences: 3, rule: 'Report the verifier result.' }),
  ];
  const packet = buildBehavioralContextPacket(candidates);
  assert.equal(packet.injected.length, 2);
  assert.equal(packet.duplicateItems, 1);
  assert.ok(!JSON.stringify(packet).includes('DO_NOT_INJECT'));
  const used = markBehavioralContextUse(packet, [{ evidenceId: packet.injected[0].evidenceId, useType: 'verification' }]);
