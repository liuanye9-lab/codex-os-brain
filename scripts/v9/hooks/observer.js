...
async function handleObservation(input, core) {
  ...
  if (!failed) {
    core.failures?.succeed?.({ operation: input.toolName });
  ...
  if (result.state.status === 'warning') {
    return {
      ...additionalContext('The same operation has failed twice. Check the error and assumptions before retrying.', input.event),
      reason_code: 'repeated_failure_warning',
    };
  }
