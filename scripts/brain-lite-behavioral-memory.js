'use strict';
const { contentHash, redactSensitive } = require('./brain-lite-common');

function redactPrivatePaths(value) {
  return String(value || '')
    .replace(/(?:\/Users\/|\/home\/)[^/\s]+(?:\/[^\s,;:)"']+)*/g, '[PRIVATE_PATH]')
    .replace(/\b[A-Za-z]:\\(?:[^\\\s]+\\)*[^\\\s,;:)"']+/g, '[PRIVATE_PATH]');
}

function sanitizePersistentText(value, maxLength) {
  return redactPrivatePaths(redactSensitive(String(value || '').normalize('NFKC')))
    .trim()
    .replace(/\s+/g, ' ')
    .slice(0, maxLength);
}
...
function normalizeRule(rule) {
  return sanitizePersistentText(rule, 1200);
}
...
function createCandidate(input = {}) {
  ...
  const scopeKey = sanitizePersistentText(input.scopeKey || event.taskFamily || 'general', 160) || 'general';
...
module.exports = { createCandidate, mergeCandidate, normalizeRule, sanitizePersistentText };
