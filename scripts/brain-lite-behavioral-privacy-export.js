...
function requireSalt(salt) {
  if (typeof salt !== 'string' || salt.length < 32) throw new Error('export salt must contain at least 32 random characters');
  if (new Set(salt).size < 10 || /(participant|password|example|specific[-_ ]?salt)/i.test(salt)) {
    throw new Error('export salt must be a strong random value, not a repeated or descriptive phrase');
  }
  return salt;
}
