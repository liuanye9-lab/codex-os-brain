...
const INJECTED_TAGS = [
  ...
  'agent-context', 'memory-context', 'tool-context', 'in-app-browser-context',
  'environment_context', 'app-context'
];

const MAX_DETECTION_CHARS = 32_768;
...
function stripInjectedContent(text) {
  let output = String(text || '').slice(0, MAX_DETECTION_CHARS);
...
function detectCorrection(input) {
  const text = stripQuotedContent(stripInjectedContent(input));
...
module.exports = { detectCorrection, stripInjectedContent, stripQuotedContent, PATTERNS };
