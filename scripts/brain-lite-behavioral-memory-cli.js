...
const { approveBehavioralCanary, evaluateBehavioralCandidate } = require('./brain-lite-behavioral-policy');
...
const USAGE = 'commands: detect, capture, evaluate, approve-canary, recall, export';
...
function main(argv = process.argv.slice(2)) {
  ...
  if (!['detect', 'capture', 'evaluate', 'approve-canary', 'recall', 'export'].includes(command)) {
  ...
  let evaluated;
  if (command === 'approve-canary') {
    if (args['confirm-canary'] !== true) throw new Error('approve-canary requires --confirm-canary');
    if (typeof payload.passed !== 'boolean') throw new Error('approve-canary requires boolean passed');
    evaluated = approveBehavioralCanary(store.candidates[index], payload.passed, policy);
  } else {
    const samples = Array.isArray(payload.samples) ? payload.samples : [];
    evaluated = evaluateBehavioralCandidate(store.candidates[index], samples, policy);
  }
