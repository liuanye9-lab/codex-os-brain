...
function sanitizeTraceEvent(event = {}) {
  ...
  for (const [key, value] of Object.entries(event)) {
    ...
    if (key === 'skillIds') { output[key] = Array.isArray(value) ? value.filter((item) => /^sk_[a-f0-9]{20}$/.test(item)) : []; continue; }
...
module.exports = { appendTraceEvent, eventId, readTrace, sanitizeTraceEvent, skillId };
